...
#ifndef _HAL_AES_F1X_H_
#define _HAL_AES_F1X_H_
...
#include "hal_aes_f_01_00_00.h"
#else
#error "define CONFIG_AES_VER_MINOR of F-type AES at config_xxx.h"
#endif

#else
#error "define CONFIG_AES_VER_EXT of F-type AES at config_xxx.h"
#endif
...
static __inline void AES_SetIntrUnMask(IRQn_Type eIrq, bool bEnable)
{
...
static __inline void AES_SetDescKeyExp(void)
//static void AES_SetDescKeyExp(void)
{
    AES_Type *ptAes = (AES_Type *)AES_REG_BASE;
    INTC_Type *ptIntc = (INTC_Type *)INTC_BASE; 
    uint32_t un32AddrReg = 0, un32KeyAddrReg = 0, un32Timeout = AES_DESC_KEY_EXPANSION_TIMEOUT;
    
    SET_AES_CR_CIPHER_MODE(ptAes, AES_KEY_EXPANSION_MODE);
    SET_INTC_IRQ_UNMASK_EN(ptIntc, false);
    ...
    SET_INTC_IRQ_UNMASK_EN(ptIntc, true);
    SET_AES_CR_CIPHER_MODE(ptAes, 1);

    for(int i = 0; i < AES_KEY_REG_COUNT; i++)
    {
        un32KeyAddrReg = (uint32_t)(GET_AES_DR_KEY_OUT_BASE_ADDR(ptAes) + i);
        un32AddrReg = (uint32_t)(GET_AES_DR_KEY_BASE_ADDR(ptAes) + i);
        *(volatile uint32_t *)un32AddrReg = *(volatile uint32_t *)un32KeyAddrReg;
    }  
}
#endif

#endif /* _HAL_AES_F1X_H_ */
