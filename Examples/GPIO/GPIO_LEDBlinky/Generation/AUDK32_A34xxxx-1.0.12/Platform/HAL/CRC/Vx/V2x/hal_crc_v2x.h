 ...
 * @brief       Private APIs for V2x typed CRC
 *
...
#ifndef _HAL_CRC_V2X_H_
#define _HAL_CRC_V2X_H_
...
#include "hal_crc_v_02_00_00.h"
#elif (CONFIG_CRC_VER_MINOR == 1)
#include "hal_crc_v_02_00_01.h"
#elif (CONFIG_CRC_VER_MINOR == 2)
#include "hal_crc_v_02_00_02.h"
#elif (CONFIG_CRC_VER_MINOR == 3)
#include "hal_crc_v_02_00_03.h"
#else
#error "define CONFIG_CRC_VER_MINOR of V-type CRC at config_xxx.h"
#endif

#else
#error "define CONFIG_CRC_VER_EXT of V-type CRC at config_xxx.h"
#endif
...
#if defined(CRC_FEATURE_INPUT_DATA_8BIT)
...
{
    return P_CRC_DMA_BUS_SIZE_BYTE;
}
#else
static __inline void CRC_SetData(CRC_Type *ptPeri, uint8_t *pun8Data, uint32_t un32Len)
{
    ...
    if(un32Remainder != 0)
    {
       for(i = 0; i < un32Remainder; i++)
       {
           un32Data |= (*pun8Data++ << ((4-un32Remainder)+i)*8);
       }
    ...
    for(i = 0; i < un32Quotient; i++)
    {
        ...
        for(j = 0; j < CRC_BYTE_ALIGN; j++)
        {
            un32Data |= (*pun8Data++ << (j*8));
        }        
...
static __inline HAL_ERR_e CRC_GetAlignData(uint8_t *pun8Data, uint32_t un32Len, uint32_t *pun32AlignData, uint32_t *pun32AlignLen)
{
    ...
    *pun32AlignLen = un32MemSize;
    ...
    if(un32Remainder != 0)
    {
       for(i = 0; i < un32Remainder; i++)
       {
           *pun32Mem |= (*pun8Data++ << ((4-un32Remainder)+i)*8);     
       }
       pun32Mem++;
    }
...
static __inline HAL_ERR_e CRC_GetSupportPoly(P_CRC_POLY_e eMode)
{
    switch(eMode)
    {
        ...
        case P_CRC_POLY_16:
        case P_CRC_POLY_8:
#if defined(CRC_FEATURE_POLY_16CCITT)
