 ...
 * @brief       Private APIs for V3x typed SCU
 *
...
#ifndef _HAL_SCU_V3X_H_
#define _HAL_SCU_V3X_H_
...
{
    FMC_Type *ptFmc = (FMC_Type *)SCU_FMC_ADDR;

    SET_SCU_CFMC_LATENCY(ptFmc, SCU_FMC_DEFAULT_LATENCY);
    SET_SCU_DFMC_LATENCY(ptFmc, SCU_FMC_DEFAULT_LATENCY);
}
...
static __inline void SCUCLK_SetFlashLatency(uint32_t un32CoreClock)
{
    ...
    {
        un8Wait = SCU_FMC_DEFAULT_LATENCY;
    }

    SET_SCU_CFMC_LATENCY(ptFmc, un8Wait);
    SET_SCU_DFMC_LATENCY(ptFmc, un8Wait);
                                         ...
                                         P_SCUCLK_DIV_e ePostMClkDiv)
{
    ...
    SET_SCU_SCCR_HCLKDIV(ptScuClk, SCUCLK_HCLK((uint8_t)ePostMClkDiv));
